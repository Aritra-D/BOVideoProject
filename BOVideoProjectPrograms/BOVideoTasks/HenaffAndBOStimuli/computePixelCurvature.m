...
function [ct_pixel, globalCurvature_imageSet, ct_pixel_pca, globalCurvature_pca_imageSet,imageLabels] = computePixelCurvature(data,imageType,imagematrix,frameFormat,numPC)
...
if strcmp(data,'HenaffStim2021')
    ...
    for iVideo = 1:size(sortedImageSet,1)
        for iPCList = 1:length(numPCList)
            clear temp_video reconstructedVideo delta_xt_pixel norm_vt_pixel moving_dotProduct_norm_vt_pixel
            ...
            norm_vt_pixel = cellfun(@(x) x./norm(x), num2cell(delta_xt_pixel,1),'UniformOutput',false); % computing the unit displacement vectors
            moving_dotProduct_norm_vt_pixel = cell2mat(cellfun(@(x,y) dot(x,y), norm_vt_pixel(1:end-1),norm_vt_pixel(2:end),'UniformOutput',false)); % dot product of sequential displacement vectors
            ct_pixel_pca(iVideo,iPCList,:) = rad2deg(acos(moving_dotProduct_norm_vt_pixel)); %#ok<*AGROW> % Calculates angle for element of the array (dot product of two successive vectors)
