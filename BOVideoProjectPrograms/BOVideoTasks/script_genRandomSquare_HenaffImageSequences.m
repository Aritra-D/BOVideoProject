%to do: different square orientations
...
delete([basedatapath '*frame*.bmp']);
delete([basedatapath '*video*.mp4']);

%% USER ADJUSTABLE PARAMETERS
RFpos_real = [-1 -0.5]; %x and y ccoordinateds of RF center in ddegrees 

repetitions = 30; %how often a single scene is repeated
...
for nvi=1:nvideos
    ...
    for i=1:nsurfaces
        squarewidths=[squarewidths randi(sizerange,1)];
        squareheights=[squareheights randi(sizerange,1)];
        colors=[colors randsample(setdiff([0:1:255],forbiddencolors),1)]; %random color, but cannot be too close to bgcolor
        xs = [xs randi([1 scenesize(2)],1)]; %initial square position (left top corner column)
        ys = [ys randi([1 scenesize(1)],1)]; %initial square position (left top corner row)
        dxs = [dxs randi(round([-1 1].*maxspeed.*ppd),1)]; %how much pixels the x coordinate of a square changes in each frame
        dys = [dys randi(round([-1 1].*maxspeed.*ppd),1)]; %how much pixels the y coordinate of a square changes in each frame
        ddxs = [ddxs randi(round([-1 1].*maxaccel.*ppd),1)]; %how much pixels the x coordinate of a square changes in each frame
        ddys = [ddys randi(round([-1 1].*maxaccel.*ppd),1)]; %how much pixels the y coordinate of a square changes in each frame
    end
    ...
    for nfi=1:nframes
        ...
        allframes=[allframes {scene_trimmed}];
    end
    v=VideoWriter([basedatapath 'video' num2str(nvi)],'Uncompressed AVI');
...
[dummy, total_pix]=angle2pix(display,fixtarget_width_total);
[dummy, center_pix]=angle2pix(display,fixtarget_width_center);
...
imwrite(matrix,[basedatapath fixfilename '.bmp']);
clear matrix;
...
imwrite(matrix,[basedatapath bgfilename '.bmp']);
clear matrix;
    ...
    for cli=1:maxcli %different luminances
        ...
        for si=1:size(sidelengths,1)
            ...
            for ri=1:numel(rotangles)
                ...
                [dummy,sidelength_pix]=angle2pix(display,currsidelength);
                sidelength_pix=round(sidelength_pix);
                [dummy,currheight_pix]=angle2pix(display,currheight);
                currheight_pix=round(currheight_pix);

                im=ones(currheight_pix,sidelength_pix); %resp H and W
                im=im.*bgval;
                [x,y]=find(im(:,:,1)==bgval);
                ...
                colorstring=colorstring(find(~isspace(colorstring)));
...
%                 end
                ...
                imwrite(uint8(currim.*255),[basedatapath currfn '.bmp']);

                  allscenes=[allscenes {currfn}];
...
imwrite(imbfin,[basedatapath 'imb' lumname '_111.bmp']);

im=imread([basedatapath 'imb' lumname '_111.bmp']);
...
for i=1:n_allen_videos
    if mod(i,2)==0
v=VideoReader('natural_movie_three.mp4');
    else
...
for ri=1:repetitions
    ...
    for pi=1:size(positions,1)
        ...
        allscenes_temp=[allscenes_temp; randsample(allscenes_temp,ntoadd)];
        end
...
savepath = basedatapath;
   ...
   fileID = fopen([savepath 'videosAndBO.txt'],'w');
    ...
    headerline = generate_condition('FID',fileID,'Header', 7);
    ...
    for i=1:size(allscenes_forcond_all,1)
        ...
        if numel(strfind(allscenes_forcond_all{i,1},'squareR'))>0
            ...
            TOno=TOno+1;
            TaskObject(TOno).Type = 'pic';
            TaskObject(TOno).Arg{1} = ['imb' lumname '_111'];
            TaskObject(TOno).Arg{2} = 0;
            TaskObject(TOno).Arg{3} = 0;
