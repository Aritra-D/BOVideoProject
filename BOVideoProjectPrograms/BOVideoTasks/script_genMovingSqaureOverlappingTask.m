...
maxspeed = 0.25; % max speed (dva per frame) % original value = 0.3
...
%BO parameters
colors_BO=[1 1 1];
rotangles=linspace(0,180,7); % border orientations %
...
if createnewvideos
    ...
    colorsUnique = [180 255]; % colors for two squares forbiddencolors: 105-156

    nvideos = length(colorsUnique)*size(rotPairs_unique,1); %size(rotrange,2);
    nvi =1; % video index
    ...
    for iRotPair = 1:size(rotPairs_unique,1)
        for iColor = 1:length(colorsUnique)
            ...
            for nfi= 1:nframes
                ...
                for iSquare=1:nsurfaces
                    iRotAngle = rotPairs_unique(iRotPair,iSquare);
                    switch iRotAngle
                    ...
                    scene = addOrientedSquare(scene,squarewidths,squareheights,dxs*(nfi-1),dys*(nfi-1),rotPairs_unique(iRotPair,iSquare),colors(iSquare),bgval);
                end
                % scene_trimmed=scene(scenesize0(1)+1:(scenesize0(1)*2),scenesize0(1)+1:(scenesize0(1)*2));
                allframes = cat(2,allframes,{scene});
            end
            v = VideoWriter(fullfile(basedatapath,'movingOverlappingSquareStimuli',['Video' num2str(nvi) '_' num2str(rotPairs_unique(iRotPair,1)) '_' num2str(rotPairs_unique(iRotPair,2)) '_' colorStr '.avi']),'Uncompressed AVI');
