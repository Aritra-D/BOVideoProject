...
delete(fullfile(basedatapath,'video_movingSquareOverlapping', '*.avi'));

%% USER ADJUSTABLE PARAMETERS
RFpos_real = [-5 -5]; %x and y ccoordinateds of RF center in ddegrees
...
sizerange_dva = 5;%[2 10]; %range of possible widths/heights of a rectangle
maxspeed = 0.2104; % max speed (dva per frame) % original value = 0.3
...
framerate = 30; %frames per second. 30 f/s is RandomSquare Videos and Allen video framerate.
nsurfacerange=[5,10]; %range of potential number of surfaces
% nvideos = 1; %number of different videos
createnewvideos=1;
n_allen_videos = 4;
...
scenespertrial = 3; %number of square scenes shown in each trial (must be 1 if npos>1)
stimdur = 500;
blankdur = 500; % duration of blank interval in between stims
...
% %Calculate pixel size in cardinal directions
% pixSize = display.width/display.res(1);   %cm/pix
% sz = 2*display.dist*tan(pi*1/(2*180));  %cm
% pix = sz/pixSize;   %pix
...
%% generate video stimuli with orthogonally moving square circles
...
scenesize0 = round(scenesize_dva.*ppd); % ppd = pixels per degree of visual angle
...
if createnewvideos
    ...
    nframes = round(videodur*framerate/1e3); % how many frames should be drawn for the video
    scenesize = flip(maxdims); % round(scenesize_dva.*ppd);
    sizerange = round(sizerange_dva.*ppd); % pixels

    relativePosBetweenSquares = 90; % The two squares should be 90 degrees apart
    rotAngleStep = 45;
    rotAngles = 360/rotAngleStep;
    rotAnglesList = 0:rotAngleStep:360-rotAngleStep;

    rotPairs = {};
    for i = 1:rotAngles % Determining circular indices
        rotPair1 = [rotAnglesList(i) rotAnglesList((mod(i+2-1,rotAngles)+1))];
        rotPair2 = [rotAnglesList(i) rotAnglesList((mod(i-2-1,rotAngles)+1))];
        rotPairs = cat(1,rotPairs,rotPair1,rotPair2);
    end

    % rotPairs_sorted = cellfun(@(x) sort(x), rotPairs, 'UniformOutput', false);
    % rotPairs_unique = unique(cell2mat(rotPairs_sorted), 'rows');

    rotPairs = cell2mat(rotPairs);

    colorsUnique = [1 255]; % colors for two squares forbiddencolors: 105-156

    nvideos = length(colorsUnique)*size(rotPairs,1); %size(rotrange,2);
    nvi =1; % video index

    disp(['Creating Video: ' num2str(nvi)])
    squarewidths = sizerange; squareheights = sizerange;
    % xs = []; ys = [];
    % dxs = []; dys = [];
    nsurfaces = 2; % number of rectangles

    for iRotPair = 1:size(rotPairs,1)
        for iColor = 1:length(colorsUnique)
            switch iColor
                case 1
                    colors = colorsUnique;
                    colorStr = '1C2W';
                case 2
                    colors = flip(colorsUnique);
                    colorStr = '1W2C';
            end
            ...
            for nfi= 1:nframes
                ...
                for iSquare=1:nsurfaces
                    iRotAngle = rotPairs(iRotPair,iSquare);
                    switch iRotAngle
                        % 8 rotation angles used for stimuli generation
                        case 0;   dxs_direction = -1; dys_direction = 0; speed = maxspeed*sqrt(2);
                        case 45;  dxs_direction = -1; dys_direction = 1; speed = maxspeed;
                        case 90;  dxs_direction =  0; dys_direction = 1; speed = maxspeed*sqrt(2);
                        case 135; dxs_direction =  1; dys_direction = 1; speed = maxspeed;
                        case 180; dxs_direction =  1; dys_direction = 0; speed = maxspeed*sqrt(2);
                        case 225; dxs_direction =  1; dys_direction =-1; speed = maxspeed;
                        case 270; dxs_direction =  0; dys_direction = -1; speed = maxspeed*sqrt(2);
                        case 315; dxs_direction = -1; dys_direction = -1; speed = maxspeed;

                            % Additional conditions if 12 rotation angles used for stimuli
                            % generation

                        % case 30; dxs_direction = -1; dys_direction{1} = 1; maxspeed = maxspeed*sqrt(2);
                        % case 60; dxs_direction = -1; dys_direction{1} = 1; maxspeed = maxspeed*sqrt(2);
                        % case 120; dxs_direction = 1; dys_direction{1} = 1; maxspeed = maxspeed*sqrt(2);
                        % case 150; dxs_direction = 1; dys_direction{1} = 1; maxspeed = maxspeed*sqrt(2);
                        % case 210; dxs_direction = 1; dys_direction{1} = -1; maxspeed = maxspeed*sqrt(2);
                        % case 240; dxs_direction = 1; dys_direction{1} = -1; maxspeed = maxspeed*sqrt(2);
                        % case 300; dxs_direction = -1; dys_direction{1} = 1; maxspeed = maxspeed*sqrt(2);
                        % case 360; dxs_direction = -1; dys_direction{1} = 1; maxspeed = maxspeed*sqrt(2);
                    end
                    dxs = round(dxs_direction.*speed.*ppd); % how much pixels the x coordinate of a square changes in each frame
                    dys = round(dys_direction.*speed.*ppd); % how much pixels the y coordinate of a square changes in each frame

                    scene = addOrientedSquare(scene,squarewidths,squareheights,dxs*(nfi-1),dys*(nfi-1),rotPairs(iRotPair,iSquare),colors(iSquare),bgval);
                end
                % scene_trimmed=scene(scenesize0(1)+1:(scenesize0(1)*2),scenesize0(1)+1:(scenesize0(1)*2));
                allframes = cat(2,allframes,{scene});
            end

            [ct_pixel(nvi,:),globalCurvature_CroppedVideo(nvi)] = computePixelCurvature_movingSquareVideos(allframes,'circular-crop',0,1,1);
            videoID{nvi} = ['Video' num2str(nvi) '_' num2str(rotPairs(iRotPair,1)) '_' num2str(rotPairs(iRotPair,2)) '_' colorStr];

            v = VideoWriter(fullfile(basedatapath,['Video' num2str(nvi) '_' num2str(rotPairs(iRotPair,1)) '_' num2str(rotPairs(iRotPair,2)) '_' colorStr '.avi']),'Uncompressed AVI');
...
disp('Summary of Pixel Curvatures of Moving Square Video (Cropped Central Region of Video coinciding with CRF of neurons')
tab = table(videoID',ct_pixel,globalCurvature_CroppedVideo');
tab.Properties.VariableNames = {'VideoID', 'Discrere Curvatures', 'Global Curvature'};
disp(tab)

    % for nvi = 1:nvideos
    %     disp(['Creating Video: ' num2str(nvi)])
    %     squarewidths = []; squareheights = [];
    %     xs = []; ys = []; dxs = []; dys = []; ddxs = []; ddys = [];
    %     nsurfaces = 2; % number of rectangles
    %     for i = 1:nsurfaces
    %         squarewidths = cat(2,squarewidths,sizerange(1));
    %         squareheights = cat(2,squareheights,sizerange(1));
    %         rotangles = cat(2,rotangles,rotrange(nvi,i));
    %         if i==1
    %             xs = cat(2,xs,0);  % initial square origin position (left top corner column)
    %             ys = cat(2,ys,0);  % initial square origin (left top corner row)
    %             dxs = cat(2,dxs,round([-1 -1].*maxspeed.*ppd)); % how much pixels the x coordinate of a square changes in each frame
    %             dys = cat(2,dys,round([0 0].*maxspeed.*ppd)); % how much pixels the y coordinate of a square changes in each frame
    %             ddxs = 0; % cat(2,ddxs,randi(round([-1 1].*maxaccel.*ppd),1)); % how much pixels the x coordinate of a square changes in each frame
    %             ddys = 0; % cat(2,ddys,randi(round([-1 1].*maxaccel.*ppd),1)); % how much pixels the y coordinate of a square changes in each frame
    %         elseif i==2
    %             xs = cat(2,xs,0); % randi([1 scenesize(2)],1)); % initial square position (left top corner column)
    %             ys = cat(2,ys,0);     % randi([1 scenesize(1)],1)); % initial square position (left top corner row)
    %             dxs = cat(2,dxs,round([0 0].*maxspeed.*ppd)); % how much pixels the x coordinate of a square changes in each frame
    %             dys = cat(2,dys,round([-1 -1].*maxspeed.*ppd)); % how much pixels the y coordinate of a square changes in each frame
    %             ddxs = 0; % cat(2,ddxs,randi(round([-1 1].*maxaccel.*ppd),1)); % how much pixels the x coordinate of a square changes in each frame
    %             ddys = 0; % cat(2,ddys,randi(round([-1 1].*maxaccel.*ppd),1)); % how much pixels the y coordinate of a square changes in each frame
    %         end
    %     end
    ...
    %                 end
    %             end
    %             % scene=addSquare(scene,squarewidths(i),squareheights(i),xs(i)+(dxs(2*i-1)*(nfi-1)),ys(i)+(dys(2*i-1)*(nfi-1)),colors(i));
    %             scene=addOrientedSquare(scene,squarewidths(i),squareheights(i),xs(i)+(dxs(2*i-1)*(nfi-1)),ys(i)+(dys(2*i-1)*(nfi-1)),rotangles(i),colors(i),bgval);
    %         end
    %         % scene_trimmed=scene(scenesize0(1)+1:(scenesize0(1)*2),scenesize0(1)+1:(scenesize0(1)*2));
    %         allframes = cat(2,allframes,{scene});
    %     end
    %     v = VideoWriter(fullfile(basedatapath,'video_movingSquareOverlapping.avi'),'Uncompressed AVI');
    %     v.FrameRate = framerate;
    %     open(v);
    %     for i=1:numel(allframes)
    %         writeVideo(v,allframes{i}./255);
...
%% MAKE BO STIMULI
timingfilename = 'movingSquareVideo_BOtask_timingFile';
...
if isscalar(unique(imb)) % we can simplify the lin_lum algorithm if it is an isoluminant field
...
allvideos=[];
for i=1:nvideos
    d=dir([fullfile(basedatapath, ['Video' num2str(i) '_*.avi'])]);
    if numel(d)~=1
        error('not found');
    else
...
for ri=1:repetitions
    ...
    temp= [allBOscenes_forcond; allvideos];
...
fileID = fopen(fullfile(savepath,'MovingSquaresAndBO.txt'),'w');
...
headerline = generate_condition('FID',fileID,'Header', 6);
...
for i=1:size(allscenes_forcond_all,1)
    ...
    if numel(strfind(allscenes_forcond_all{i,1},'squareR')) % || numel(strfind(allscenes_forcond_all{i,1},'Henaff_stim'))>0
        ...
        for scenei=1:scenespertrial
            ...
            TaskObject(TOno).Arg{3} = RFpos_real(2)+fixbias(2);
        end
        
        % Background for Interstim (blank)
        TOno=TOno+1;
        TaskObject(TOno).Type = 'pic';
        TaskObject(TOno).Arg{1} = ['imb' lumname '_111']; %['imb' lumname '_111'];
